function loadPage(pageNo=1){
	...
	$.get("data/products/getProductsByKw.php",query)
		...
		for(var p of data){
			html+=`<li>
					<h3><a href="product_details.html?cid=${p.cid}">${p.title}</a></h3>
					<p><span>${p.boxes}</span><span>${p.spec}</span><span>乘坐${p.capacity}人</span></p>
					<a href="product_details.html?cid=${p.cid}">
					  <img src="${p.md}" alt="">
					</a>
					<hr>
					<div class="carState">
						<div>日均价</div><span>${p.price}</span>
						<div>总价价</div><span>${p.price*2}</span>
						<div>
